...
phase_name=""
phase_desc=""
progress_msg=$(printf '%s' "$file_path" | python3 -c '
import sys, re, os
...
# Phase document pattern: 01-spec.md, 02-design.md, 03-plan.md, 04-review.md, 05-ship.md
phase_map = {
...
if basename in phase_map:
    ...
    for i, part in enumerate(parts):
        if part == "features" and i > 0 and parts[i-1] == "docs":
            found = True
            break
    if found or basename in phase_map:
...
message="[progress] 产出 ${basename} \u2014 阶段链进展至 ${phase} (${desc})"
