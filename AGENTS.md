...
Unified 会为阶段文档自动维护项目级 feature 状态：

- canonical path: `docs/features/<feature>/state.json`
- 写入 `00-brainstorm.md` 到 `05-ship.md` 时，`doc-tracker` 会更新当前阶段、最后阶段文档、下一步命令和最后活动时间。
- 新 session 的 `SessionStart` 会读取最新 active feature state，并注入短恢复提示；这只是恢复提示，不会自动激活 Unified runtime，也不会自动开始实现。
- `state.json` 只记录可随项目走的事实：`feature_path`、`branch`、`current_stage`、`last_phase_doc`、`next_command`、`last_activity`、`stale_reason`。
- 禁止把本地瞬时状态写进 `state.json`，包括 `dirty_status`、branch match 结果、机器路径、用户名、主机名。
- `/save` 和 `/restore` 仍用于 decision-rich checkpoint：记录为什么这么做、剩余工作和注意事项；基础阶段连续性不依赖手动 `/save`。
...
| `/design` | design-workflow-design + artifact_type 对应 design 技能（canonical delivery class: `software / content / visual`）+ codex-rescue（可选，视觉 mockup） | 证据驱动的创作设计定稿 + Codex 视觉 mockup（可选） + 项目级约束同步 | `docs/features/YYYYMMDD-<name>/02-design.md` + `assets/mockup-direction-{1,2,3}.png` + `assets/design-tokens-extracted.json`（可选） + `DESIGN.md` |
| `/plan` | build-workflow-plan | 任务计划 + 项目级文档同步计划 | `docs/features/YYYYMMDD-<name>/03-plan.md` |
| `/build` | build-workflow-execute + artifact_type 对应技能 | 软件/内容/视觉产物+验证+ADR | `docs/features/YYYYMMDD-<name>/adr/` |
...
对普通 Codex 对话，默认保持 direct mode，不自动加载 router 或阶段技能。只有显式进入 Unified 工作流时，才激活 router-first discovery 和 loading tier。
...
**重要差异：** careful hook 在 Codex 上对不可逆操作使用 fail-closed 模式（阻止破坏性命令而非提示确认），因为确认型交互语义在 Codex 上并不稳定；对显式限定在生成目录内的清理命令可按条件放行，避免把常见维护动作一并卡死。
