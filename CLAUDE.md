...
> 宪法 + 47 技能 + 11 命令 + 22 角色（15 审查 + 7 核心工程）= 按阶段加载的多产物开发技能套件。支持 Claude Code 和 Codex CLI。
...
│   └── reflect/             复盘（2）
│
├── commands/                11 命令入口（每个 .md 定义命令行为）
├── .agents/skills/           11 命令入口（Codex CLI skill 命令）
...
verify/    → review（审查）、spec-compliance（规格符合性）、code-quality（代码质量）、
             debug（调试）、accessibility（无障碍）、integration-testing（集成测试）、
...
| `/review` | verify-workflow-review + spec-compliance + code-quality | 两阶段审查报告 | `docs/features/YYYYMMDD-<name>/03-review.md` |
...
- 调用技能前先通读整个技能
- 实现非平凡变更前先陈述假设
- 需要用户输入时，优先使用宿主环境的结构化提问工具；没有该工具时，用一条简短纯文本问题降级
- 功能变更后必须同步更新版本号（`package.json` + `.claude-plugin/plugin.json` + `.codex-plugin/plugin.json`），不能等到 push 才改

### 绝不能做
- 不能一次罗列多个问题让 human partner 逐条回答——宪法第 9 条要求一次只问一个问题
...
- **Claude Code**: `skills/` 是真实技能目录，通过 `.claude-plugin/` 自动注册 hooks 和 commands。`commands/` 下是斜杠命令入口。
- **Codex CLI**: `.agents/skills/` 是 11 命令薄包装入口 + 47 技能薄包装入口，通过 `.codex-plugin/` 注册 skills，通过 `.codex/hooks.json` 注册 hooks（需 `[features] codex_hooks = true`）。Codex 使用 `$command` 语法。

Hooks 跨平台行为差异：
- **careful.sh** — Claude Code: `permissionDecision: "ask"`（提示确认）；Codex: `permissionDecision: "ask"` 解析但 fail-open（破坏性命令不会暂停）。等 Codex 完全支持 "ask" 后自动生效。
- **freeze.sh** — 两平台均使用 `permissionDecision: "deny"`（阻止范围外编辑），完全生效。
- **session-start.sh** — 两平台输出格式相同（`hookSpecificOutput.additionalContext`），完全生效。
