...
- **Codex CLI**: `.agents/skills/` 是 10 命令薄包装入口 + 44 技能薄包装入口，通过 `.codex-plugin/` 注册 skills，通过 `.codex/hooks.json` 注册 hooks（需 `[features] codex_hooks = true`）。Codex 使用 `$command` 语法。
