...
> 宪法 + 45 技能 + 11 命令 + 22 角色（15 审查 + 7 核心工程）= 按阶段加载的多产物开发技能套件。支持 Claude Code 和 Codex CLI。
...
- **Codex CLI**: `.agents/skills/` 是 11 命令薄包装入口 + 45 技能薄包装入口，通过 `.codex-plugin/` 注册 skills，通过 `.codex/hooks.json` 注册 hooks（需 `[features] codex_hooks = true`）。Codex 使用 `$command` 语法。
