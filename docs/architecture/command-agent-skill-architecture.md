...
## 当前合同补充：Risk-Based Role Escalation（2026-05-13）

当前实现不再把 Army 理解为每阶段固定全员广播。`AGENTS.md` 是权威入口：小型变更可跳过 army，标准变更使用最小必要角色，高风险按 UI/security/performance/test/docs 等维度追加角色，只有 `--full`、大型/高风险、对抗性审核或发版前才全开。本文历史章节中的固定数量示例只作为旧设计背景，不代表当前执行合同。
...
```yaml
# Command: /plan
Phase 1: Parse Spec + Design → Agent: task-planner → Output: 03-plan.md (draft)
Phase 2: Multi-Role Review (parallel) → Agents: 4 reviewers → Output: review-comments.md
Phase 3: Refine Plan → Agent: task-planner → Output: 03-plan.md (final)
```
...
**示例（`--full` 时并行分派全部 plan reviewers）：**

```
在同一条 assistant 消息中，同时发起已选 Agent 工具调用：
  - Agent 1: plan-ceo-reviewer — 审查商业价值
  - Agent 2: plan-eng-reviewer — 审查工程可行性
  - Agent 3: plan-design-reviewer — 审查 plan 是否正确消费 design
  - Agent 4: plan-security-reviewer — 审查安全合规

每个 Agent 独立上下文运行，产出各自的审查报告。
已选 Agent 完成后，在主 session 中合并为统一反馈文档。
```
...
**具体要求：**
- 每个 Phase 的产出必须写入文件（如 `02-design.md`、`03-plan.md`、`04-review.md`）
...
---
读取 AGENTS.md → 加载 `skills/define-workflow-refine/SKILL.md`。
```

**实际行为：** 将所有逻辑委托给 `define-workflow-refine` 技能，包括：
- 需求澄清（5W1H 提问）
- External Scan（按 artifact_type 搜索已有方案）
- Idea Scout Army 按风险升级审查（已选 scouts 并行）
- 需求文档生成
...
---
读取 AGENTS.md → 加载 `skills/build-workflow-plan/SKILL.md`。
```
...
- Plan Review Army 按风险升级审查（已选 reviewers 并行）
- 计划迭代
...
---
读取 AGENTS.md → 加载 `skills/build-workflow-execute/SKILL.md`。
```
...
---
读取 AGENTS.md → 加载 `skills/verify-workflow-review/SKILL.md`。
```
...
- visual/deck → 视觉审查
- Review Army 按风险升级分派（已选 reviewers 并行）
...
---
读取 AGENTS.md → 加载 `skills/ship-workflow-ship/SKILL.md`。
```

**实际行为：** 委托给 `ship-workflow-ship` 技能：
- Phase A: 准备发布
- Phase B: Ship Audit Army 按风险升级审计（已选 auditors 并行）
- Phase C: 导出/发布
- Phase D: 文档同步
...
---
读取 AGENTS.md → 加载 `skills/maintain-workflow-context-save/SKILL.md`。
```
...
---
读取 AGENTS.md → 加载 `skills/maintain-workflow-context-restore/SKILL.md`。
```
...
---
读取 AGENTS.md → 加载 `skills/maintain-workflow-learn/SKILL.md`。
```
...
**artifact_type 路由逻辑：**
```
software  → 两阶段审查（Spec Compliance → Code Quality）+ 按风险升级的 Review Army
document  → 内容审查
article   → 内容审查
deck      → 视觉审查
visual    → 视觉审查
...
artifact_type
    ...
    │       software  → 两阶段审查 + 按风险升级的 Review Army
    │       document  → 内容审查
    │       visual    → 视觉审查
    │
...
当前合同采用 Risk-Based Role Escalation：每个阶段先由 workflow skill 判断最小必要角色，小型变更可跳过 army，标准变更只启用核心视角，高风险按 UI/security/performance/test/docs 等维度追加角色；只有 `--full`、大型/高风险、对抗性审核或发版前才全开。并行 fan-out 只用于已选角色，不要求未选角色产出占位反馈。
...
**并行发起方式：** 主 session 在单个 assistant turn 中发起已选 Agent 调用。
...
Command: /plan
  └── Skill: build-workflow-plan
        └── 按风险选择并行分派 Agents:
              ...
              ├── plan-design-reviewer   → design 约束覆盖、任务映射、错误下沉检查
...
| Army | 角色选择策略 | 并行位置 | 反馈分级 |
|------|-----------|---------|---------|
| Review Army | 按 review-guidance 风险触发，`--full` 全开 | verify-workflow-review 内部 | Blocking / Important / Suggestion |
| Plan Review Army | 小型可跳过，标准 CEO + Eng，高风险全开 | build-workflow-plan 内部 | 同上 |
| Refine Scout Army | 小型可跳过，标准 CEO + Eng，大型 UI/合规全开 | define-workflow-refine 内部 | 同上 |
| Ship Audit Army | 标准 security + docs，UI/performance 按风险追加，`--full` 全开 | ship-workflow-ship 内部 | 同上 |

**关键问题：** 角色选择策略由 Skill 内部实现，Command 层必须声明自己遵循阶段技能的最少触发条件，而不是写死所有角色全部参与。
...
output: docs/features/YYYYMMDD-<name>/03-plan.md
---
```
...
Plan Review Army:
  ...
  design-reviewer               ← phase-lens-role ✅
...
Validation:
  - [ ] 已选 Scouts 全部完成，或记录跳过 Scout Army 的理由
  - [ ] Blocking issues 已识别
...
- [ ] artifact_type 已声明
- [ ] 经最小必要视角审查，或记录跳过 Scout Army 的理由
- [ ] 所有 Blocking issues 已解决
- [ ] 用户已批准 spec
...
Process:
  ...
  5. 估算复杂度
Output:
  - docs/features/YYYYMMDD-<name>/03-plan.md（draft）
...
Validation:
  - [ ] 已选 Reviewers 全部完成，或记录跳过 Plan Review Army 的理由
  - [ ] Blocking issues 已识别
  - [ ] 反馈按严重度分级
...
Process:
  ...
  4. 更新任务列表和依赖关系
Output:
  - docs/features/YYYYMMDD-<name>/03-plan.md（final）
...
## Exit Conditions
- [ ] 03-plan.md 存在
- [ ] 经最小必要视角审查，或记录跳过 Plan Review Army 的理由
- [ ] 所有 Blocking issues 已解决
- [ ] 用户已批准 plan
...
Validation:
  - [ ] 已选 Reviewers 完成，或记录标准模式未派发专业 reviewer 的理由
  - [ ] 反馈已按严重度分级
...
Process:
  ...
  3. 生成统一审查报告
Output:
  - docs/features/YYYYMMDD-<name>/04-review.md

Validation:
  - [ ] 报告包含所有已选 Reviewer 的反馈，或记录标准模式未派发专业 reviewer 的理由
...
## Exit Conditions
- [ ] 04-review.md 存在
- [ ] 已选 Reviewers 已完成，或记录标准模式未派发专业 reviewer 的理由
- [ ] Blocking issues 已识别
- [ ] 反馈已分级
...
Validation:
  - [ ] 已选 Auditors 全部完成，或记录跳过 Audit Army 的理由
  - [ ] 无新的 Blocking issues
...
Process:
  ...
  2. 记录发布信息
Output:
  - docs/features/YYYYMMDD-<name>/05-ship.md
...
- [ ] 发布/导出成功
- [ ] README.md 已更新
- [ ] 已选 Auditors 通过，或记录跳过 Audit Army 的理由
...
5. /review → 五轴审查 → Review Army 并行审查 → 04-review.md
6. /ship   → 发布审计 → Ship Audit Army 并行审计 → 05-ship.md
...
```
验证项：
1. Review Army 已选 Agent 可以并行分派
2. Plan Review Army 已选 Agent 可以并行分派
3. Refine Scout Army 已选 Agent 可以并行分派
4. Ship Audit Army 已选 Agent 可以并行分派
5. 每个 Agent 独立上下文运行
...
测试步骤：
  ...
  3. Phase 2：Plan Review Army 按风险升级审查
  4. Phase 3：task-planner 根据 feedback 迭代 plan

预期结果：
  - 03-plan.md 已生成
  - 已选 Reviewers 的反馈已合并，或记录跳过 Plan Review Army 的理由
  - 所有 Blocking issues 已解决
```
...
测试步骤：
  ...
  3. Phase 2：Review Army 按风险升级审查
  4. Phase 3：合并反馈生成 04-review.md

预期结果：
  - 04-review.md 已生成
  - 已选 Reviewers 的反馈已合并，或记录标准模式未派发专业 reviewer 的理由
  - 反馈按严重度分级
```
...
测试步骤：
  1. 用户输入 /ship
  2. Phase 1：发布准备
  3. Phase 2：Ship Audit Army 按风险升级审计
  4. Phase 3：发布/导出
  5. Phase 4：文档同步
...
| 测试场景 | 角色选择 | 并行模式 | 验证指标 |
|---------|-----------|---------|---------|
| Review Army | 按 review 风险触发 | parallel dispatch | 已选 Agent 同时启动 |
| Plan Review Army | 按 plan 最少触发条件 | parallel dispatch | 已选 Agent 同时启动 |
| Refine Scout Army | 按 refine 最少触发条件 | parallel dispatch | 已选 Agent 同时启动 |
| Ship Audit Army | 按 ship 最少触发条件 | parallel dispatch | 已选 Agent 同时启动 |
...
| # | 检查项 | 预期 |
|---|--------|------|
| 6 | Agent 目录与 README 一致 | 由目录和 README 表格校验 |
| 7 | Command 目录与命令映射一致 | 由 commands/ 目录和 validate 校验 |
| 8 | Skill 目录与索引一致 | 由 skills-index.json 和 validate 校验 |
| 9 | 重命名映射完整 | 新旧名映射无悬空引用 |
...
## Entry: 04-review.md (no blocking)
## Exit: 05-ship.md + README.md
...
| **Review Army** | Review 阶段按风险选择的并行审查角色组，`--full` 时全开 |
| **Plan Review Army** | Plan 阶段按风险选择的计划审查角色组，`--full` 或大型/安全/合规时全开 |
| **Refine Scout Army** | Refine 阶段按风险选择的想法侦察角色组，大型 UI/合规需求时全开 |
| **Ship Audit Army** | Ship 阶段按风险选择的发布审计角色组，`--full` 或高风险发版时全开 |
