...
1. Codex CLI v0.124+ hooks are stable enough to ship — `permissionDecision: "deny"` is enforced，当前以 fail-closed 合同为准
...
- Codex `PermissionRequest` event for careful — 历史方案曾评估过 `ask` 语义，但当前实现统一使用 PreToolUse `deny`
...
```json
{
  "name": "unified",
  "version": "2.6.0",
  "description": "统一开发技能套件 — 宪法 + 当时旧版技能与命令统计，现已过期",
  "author": {
    "name": "ZeroZ-lab",
  ...
  "interface": {
    ...
    "shortDescription": "宪法 + 当时旧版统计，现已过期"
  }
}
```
...
**Historical note**: 这份 spec 曾记录过 `ask` 语义的实验结论；当前仓库已改为 Codex 上使用 `permissionDecision: "deny"`，不再依赖该实验路径。
...
|------|----------|------------|
| 历史上曾评估 `permissionDecision: "ask"` 语义 | Medium | 该方案已废弃，当前实现统一为 `deny`；freeze.sh 仍保持 `deny`。 |
