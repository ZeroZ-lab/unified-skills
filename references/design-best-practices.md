...
5. **Local Project Truth**
   ...
   - 项目根 `DESIGN.md` 是此层的结构化载体（Google Stitch token 格式）。Phase 2 扫描时，如果 DESIGN.md 存在，必须读取其 YAML token 和 Markdown 章节作为 Local Project Truth 的基础输入。
...
项目根 `DESIGN.md` 存放跨 feature 的设计约束——视觉识别、交互模式、信息架构、品牌语气、动画运动和可访问性标准。采用 Google Stitch token 格式（YAML front matter + Markdown 章节），AI agent 可直接通过 `{colors.primary}` 语法引用。

与 `02-design.md` 的关系：
- `02-design.md`：feature-specific，每次 `/design` 生成一份，存放在 `docs/features/YYYYMMDD-<name>/` 下
- `DESIGN.md`：project-wide，项目根目录唯一一份，跨所有 feature 持久存在

数据流向：
- `/design` Phase 2（Best-Practice Scan）→ 读取 DESIGN.md 作为 Local Project Truth 输入
- `/design` Phase 6（Sync）→ 从已批准的 02-design.md 提取项目级约束写入 DESIGN.md
