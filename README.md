...
宪法 + 45 技能 + 11 命令 + 22 角色 = 按阶段加载的多产物 AI 开发技能套件。支持 Claude Code 和 Codex CLI。
...
| maintain 维护 | 6 | observability（可观测性）、deprecation-migration（废弃迁移）、context-save（保存上下文）、context-restore（恢复上下文）、learn（跨 session 学习）、goal（目标管理） |
...
**Unified 不追求更"多"，而是追求更"一致"。** 45 技能、11 命令、22 角色共享同一套宪法、同一套命名、同一套验证。
...
**Q: 技能目录为什么不用嵌套结构？**
A: 45 技能不需要三层嵌套。扁平命名 `build-quality-tdd`、`build-content-writing` 已包含完整语义。
