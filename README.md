...
它不是某一个技能的说明书，而是一个跨 Claude Code 与 Codex CLI 的整体架构：**宪法 + 12 命令 + 24 角色 + 54 技能 + Hooks 护栏 + 文档产物链**共同组成一套可验证、可扩展、可迁移的多产物开发系统。
...
这只启用 Codex hooks；也可以用 `codex --enable hooks` 做一次性启用。是否把 Unified 作为插件安装或让当前项目消费它，取决于你的 Codex 插件加载方式；本仓库提供的 Codex 插件入口是 [`.codex-plugin/plugin.json`](.codex-plugin/plugin.json)。
...
│ 54 个 SKILL.md 提供入口/出口、流程、红旗、常见说辞和验证清单             │
└───────────────────────────────────┬───────────────────────────────────┘
                                    │
┌───────────────────────────────────▼───────────────────────────────────┐
│ Artifact 层：可追踪产物链                                               │
...
| design | 7 | 用外部证据和本地事实定交互、视觉、排版、剧本、导演方案 |
...
|------|-------------|-----------|----------|
| SessionStart | 自动注入宪法 + 命令上下文 | 自动注入宪法 + 命令上下文（需 `hooks = true` 或 `--enable hooks`） | 启动时建立共同上下文 |
