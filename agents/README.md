...
`/brainstorm` 使用多席位并行脑暴：4 个不同视角的 scout 同时发散探索，然后由 facilitator 收敛整合。
...
## Brainstorm Scout Army（脑暴阶段）

| Agent | 职责 | 调用时机 |
|-------|------|---------|
| brainstorm-tech-scout | 技术视角：架构方案、技术选型、实现路径、技术风险、创新机会 | /brainstorm Phase 2（根据 profile 选择） |
| brainstorm-design-scout | 设计视角：用户旅程、交互模式、信息架构、情感设计、边缘场景 | /brainstorm Phase 2（根据 profile 选择） |
| brainstorm-business-scout | 商业视角：价值主张、市场定位、商业模式、增长路径、验证策略 | /brainstorm Phase 2（根据 profile 选择） |
| brainstorm-content-scout | 内容视角：叙事结构、受众共鸣、表达方式、传播路径、内容格式 | /brainstorm Phase 2（根据 profile 选择） |
| brainstorm-data-scout | 数据视角：数据建模、存储策略、查询模式、数据一致性、数据治理 | /brainstorm Phase 2（根据 profile 选择） |
| brainstorm-security-scout | 安全视角：威胁模型、攻击面、防护策略、合规要求、风险缓解 | /brainstorm Phase 2（根据 profile 选择） |
| brainstorm-outlier-scout | 边缘视角：反向思考、黑天鹅事件、跨界借鉴、激进简化、故意破坏 | /brainstorm Phase 2（默认参与，可显式排除） |

**脑暴阶段特点：**
- **按需选座**：根据任务类型（`--profile`）或自定义（`--seats`）选择 scout 组合
- **并行执行**：所有选中的 scout 同时工作，互不干扰
- 每个 scout 使用各自的专业框架发散
- outlier-scout 默认参与（除非显式 `--no-outlier` 排除）
- Phase 3 进行交叉 fertilization 和辩论
- Phase 4 由 facilitator 收敛整合

**可用预设配置：**
- `general` — 通用脑暴（tech + design + business + outlier）
- `tech_architecture` — 技术架构讨论（tech + data + security + outlier）
- `product_strategy` — 产品战略讨论（business + design + content + outlier）
- `content_marketing` — 内容营销脑暴（content + design + business + outlier）
- `api_design` — API 设计讨论（tech + data + security + outlier）
- `security_review` — 安全策略讨论（security + tech + data + outlier）
- `user_experience` — 用户体验讨论（design + content + business + outlier）
- `data_strategy` — 数据策略讨论（data + tech + security + outlier）

详见 `commands/brainstorm-menu.json`
