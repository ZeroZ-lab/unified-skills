...
Capture decision-rich work context as a restorable checkpoint.

Basic stage continuity is automatic when hooks are enabled: feature phase documents update `docs/features/<feature>/state.json`, and SessionStart can surface the current stage and next command. `/save` is still the explicit tool for preserving decisions, trade-offs, warnings, and remaining work that cannot be inferred from phase documents.
...
- → 新 session 会先看到 feature `state.json` 的阶段级恢复提示；需要完整决策上下文时再 `/restore`
