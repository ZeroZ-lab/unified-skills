...
AA 是可以达到的最低商业标准。AAA 在特定方面努力但不作为全局要求。
...
| 说辞 | 现实 |
|------|------|
| "可访问性最后加" | 从第一个组件做起。最后加 = 永远不会加 = 重新写一遍。 |
| "用户基本没有残障人士" | 可访问性也服务所有人：键盘用户、移动端、慢网络、临时障碍（手臂骨折）。（可访问性提升所有用户的体验） |
| "自动检查通过了就行" | aXe 能检测 ~30% 的问题。键盘手动测试和屏幕阅读器手动测试不可替代。 |
| "用 div 比用 button 方便" | div 没有键盘操作、没有 ARIA role、没有焦点管理。用 button。 |
| "颜色区分足够" | ~8% 的男性有某种形式的色弱。信息永远不只通过颜色传达。 |
