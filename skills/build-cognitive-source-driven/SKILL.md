...
| 说辞 | 现实 | 后果 |
|------|------|------|
| "我知道这个 API" | 可能是上个版本的记忆。验证 > 信任记忆。 | 上版本签名差异导致运行时崩溃，回滚成本 ≥ 1 小时 |
| "代码比文档好，直接读源码" | 源码告诉你"是什么"，文档告诉你"为什么"和"做法"。先文档，再源码。 | 遗漏语义约束和副作用，隐式 bug 在上线后爆发，排查 ≥ 4 小时 |
| "文档太长懒得看" | 用搜索定位到具体章节。不需要从头读到尾。 | 错误参数顺序或遗漏必填字段，编译通过但行为错误，浪费 ≥ 30 分钟调试 |
| "这只是一个简单的 API 调用" | 简单调用也有版本差异。30 秒查文档 > 30 分钟调 bug。 | 版本差异导致隐式行为变更，测试未覆盖边界时上线后用户报错 |
| "类型定义就够了不用看文档" | 类型定义不含语义约束、副作用、废弃说明。读文档。 | 使用已废弃 API，下版本升级时全量重构，改动量 ≥ N 处调用点 |
| "我在另一个项目用过这个 API" | 版本可能不同。API 签名可能变了。验证。 | 跨项目版本差异导致参数不匹配，生产环境报错修复 ≥ 2 小时 |
| "CHANGELOG 太长了不想看" | 那正是 bug 藏身的地方。看 Breaking Changes 章节。 | 遗漏 breaking change，升级后批量运行时错误，影响 ≥ 所有使用该 API 的模块 |
| "这个框架很简单不需要文档" | 简单框架也有陷阱。React 的 useEffect 依赖数组就是反例。 | 隐式陷阱导致性能退化或内存泄漏，用户量增长后才暴露，修复成本 ≥ 1 周 |
| "示例代码能跑就行" | 能跑 ≠ 正确。示例可能用的是已废弃 API。 | 废弃 API 在下一版本移除后紧急修复，影响 ≥ 该功能所有下游依赖 |
| "官方文档有时候也是错的" | 个别错误不构成跳过文档的理由。发现文档错误时在 CITE 中标注。 | 跳过文档导致无来源验证的代码，审查时无法通过，返工 ≥ 重写该模块 |
...
每个经过 Source-Driven 流程的模块应产出以下结构（可在代码注释或 ADR 中记录）：
...
| 决策点 | API 签名 | 来源等级 | 引用 URL | 偏离说明 |
|--------|----------|----------|----------|----------|
| [具体决策] | [验证过的签名] | P0/P1/P2/P3 | [完整 URL] | 无 / [偏离原因] |

**冲突记录**（如有）:
- [冲突描述] → [处理方式: 按最新文档修正 / 找到偏离原因 / 标记 Unknown]

**未验证项**（如有）:
- [未验证决策] → 标记 Unknown，待人类伙伴确认
```
