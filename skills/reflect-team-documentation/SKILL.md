...
默认 feature 工作只更新 `feature docs`。只有长期真相变化时，才同步 `root docs` 或 `project docs`。
...
**不要把 project docs 当 feature archive。** feature 过程留在 `docs/features/`；project docs 只保留当前长期真相。
