...
- **需读取**: `scripts/design-preview.mjs`、`references/design-best-practices.md`、`references/design-inspiration-catalog.md`、项目根 `DESIGN.md`（如果存在）
...
| 说辞 | 现实 |
|------|------|
| "浏览器里选了就算批准了" | 错。视觉对比只是方向选择，Phase 5 还要确认证据质量和完整性。 |
| "不用多轮，一轮就够了" | 可以。用户可在任意轮次说 "就这个了" 跳过后续对比。 |
| "HTML 太简陋了不好看" | 对比页面的目的是让用户区分方向差异，不是产出最终 UI。线框级足够。 |
| "直接在终端里看就行了" | 对文本产物可以。对 UI/visual/deck，视觉对比比文字描述效率高一个数量级。 |
