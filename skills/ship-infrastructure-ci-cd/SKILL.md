...
- **前置加载**: CANON.md
- **输出路径**: 工作管道定义 + 所有质量门通过 → `ship-workflow-ship`（继续发布流程）
...
| "CI 绿了就安全了" | 绿 CI ≠ 好测试。可能测试覆盖不足或有 allow_failure。 | 绿 CI + 低覆盖率 = 虚假安全感。覆盖 < 50% 时，重大 bug 在绿 CI 下照样上线，因为关键路径没有被测试。上线后 30% 的 bug 来自未覆盖路径。 |
| "E2E 太慢了不需要" | 关键用户流程必须有 E2E。只跑关键路径，不要全覆盖。 | 没有 E2E 的关键流程（支付/注册/登录）→ 用户在真实浏览器中的第一个动作就失败。上线后 40% 的生产故障来自关键流程浏览器兼容性问题。 |
| "Flaky test 是别人的问题" | Flaky test 侵蚀团队信任。谁发现谁修，或标记并创建 issue。 | Flaky test 不修 → 团队习惯忽略 CI 失败 → 真实失败被当作 flaky 忽略 → 生产 bug。当 flaky rate > 5% 时，团队对 CI 的信任度降至 < 30%。 |
...
| "安全审计步骤加到 CI 太慢了" | 安全审计在 CI 中并行运行，不增加关键路径时间。 | 不在 CI 中跑安全审计 → 已知 CVE 悄悄进入生产 → 供应链攻击风险持续累积。平均每个项目每月引入 2-3 个已知 CVE，发现滞后 30+ 天。 |
...
```markdown
# CI/CD Pipeline Report — 持续集成与持续部署管道报告

## 元信息
- **配置者**: [姓名/角色]
- **项目**: [项目名]
- **配置日期**: YYYY-MM-DD
- **平台**: [GitHub Actions / GitLab CI / 其他]

## Quality Gate 状态

| Gate | 状态 | 阈值 | 实际值 | 备注 |
|------|------|------|--------|------|
| Lint | ✅ / ❌ | 零 error | [实际 error 数] | — |
| Type Check | ✅ / ❌ | 零 error | [实际 error 数] | — |
| Unit Tests | ✅ / ❌ | 覆盖率 ≥ 80% | [实际覆盖率] | — |
| Build | ✅ / ❌ | 成功构建 | [构建时间] | — |
| Integration Tests | ✅ / ❌ | 全部通过 | [通过/总数] | — |
| E2E Tests | ✅ / ❌ | 关键路径通过 | [通过/总数] | — |
| Security Audit | ✅ / ❌ | zero high/critical CVE | [high/critical 数] | — |
| Bundle Size | ✅ / ❌ | ≤ [预算]KB | [实际大小]KB | — |

## CI 配置清单
- [ ] 缓存配置（node_modules/.cache）
- [ ] 所有 job 有 timeout-minutes
- [ ] 密钥使用 Secrets（无硬编码）
- [ ] Action 版本固定（@v4 或 @sha256，非 @main）
- [ ] 失败时输出诊断信息

## 部署策略
- **策略类型**: [Preview / Blue-Green / Staged Rollout / Rolling Update]
- **Staging 先于生产**: ✅ / ❌
- **回滚方案**: [描述或命令]
- **Feature Flag**: ✅ 已配置 / ❌ 未配置

## 性能指标
- **总 CI 运行时间**: [分钟]
- **关键路径时间**: [分钟]
- **优化建议**: [如有 > 10min，列出优化方向]

## 审查结论
- **通过**: 所有质量门 ✅ + 部署策略明确 + 回滚方案就绪
- **阻止**: 任何质量门 ❌ 或回滚方案缺失
