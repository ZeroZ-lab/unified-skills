...
- 只读 `03-plan.md` 时，按 `### Task N` 顺序直接执行
- 存在 `plans/*.md` 时，按文件编号顺序执行子计划，并在每个子计划内按 `### Task N` 顺序执行
...
- **计划拓扑优先** — 如果 `03-plan.md` 定义了 `Plan Topology`，按拓扑执行，不重新发明执行顺序
- **Task-by-task 优先** — 如果计划已经列出 `### Task N`，按 task queue 执行；不要把多个 task 合并成一个模糊切片
...
- **PLAN GAP（任务缺失、验收标准缺失、Write Scope 不足）** → STOP。记录具体缺口，回到 `/plan` 修补；不要在 build 中临时扩写任务列表。
- **scope creep（发现 "顺便" 多做一个功能）** → 记录为 "NOTED"，不移出当前任务范围。Scope Discipline > 效率假象。
- **预计耗时翻倍** → 和用户沟通："这个任务比预期复杂，预计需要 X 时间，OK 吗？"

## 纪律规则

### Rule 0：Simple First（CANON.md 第 2 条）
...
### Rule 0.5：Scope Discipline（CANON.md 第 3 条）
