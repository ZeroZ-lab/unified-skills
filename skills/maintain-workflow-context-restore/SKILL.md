...
- **出口**: 上下文摘要呈现给用户
- **指向**: 认后 → 继续原工作流（build / verify / ship 等）
...
1. **继续工作** — 按 checkpoint 的 Next Command 和 Remaining Work 继续
...
- 呈现摘要时省略 Remaining Work / Next Command / Validation State — 用户无法判断"接下来做什么、是否可继续"，恢复就失去意义。
...
- [ ] 核心 Markdown 章节全部存在（Summary、Current Objective、Git Snapshot、Progress State、Decisions Made、Validation State、Remaining Work、Blockers、Next Command、Handoff Risks、Notes）
- [ ] branch 匹配或已发出警告
- [ ] next_command 已呈现或标记为 legacy 缺失
- [ ] Validation State 已呈现，未跑检查明确标记
- [ ] Blockers 和 Handoff Risks 已高亮
- [ ] 用户确认继续（未自动开始操作）
- [ ] 呈现了摘要而非全文（用户未要求时）
