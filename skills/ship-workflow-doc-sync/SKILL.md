...
| 说辞 | 现实 | 后果 |
|------|------|------|
| "文档以后再更新" | "以后"永远不会来。代码变更时同步更新文档成本最低。事后补文档 = 重新理解代码。 | 事后补文档耗时 ×3-5；新人按旧文档操作 = 环境搭建失败 + 2 小时浪费 |
| "CHANGELOG 自己写就行" | AI 润色措辞，但变更的业务意义只有 human partner 知道。AI 不替人写叙事。 | 叙事不准确 → 用户误解变更影响 → 升级决策失误 |
| "README 不需要那么详细" | README 是新人的第一个文件。少一个步骤 = 新人多花一小时摸索。 | 每个新人多花 1 小时 × 10 个新人 = 10 小时团队浪费 |
| "这个文档没人看" | 没人看是因为过时了。保持准确的文档会被人发现和使用。 | 过时文档 → 信任崩塌 → 团队不再参考任何文档 → 知识全靠口头传递 |
| "自动更新就行，不用问" | 事实自动更新。叙事、判断、理由不能。混淆两者 = 文档失去人的视角。 | 自动写叙事 → 措辞不符真实意图 → 文档变成谎言 |
...
| 验证项 | 失败表现 | 处理方式 |
|--------|----------|---------|
| 变更文件识别不全 | 部分合并文件未被发现 | 扩展 git diff 范围；检查 submodule 和生成文件 |
| 事实性更新未执行 | 路径/版本号/数量仍不一致 | 立即修正；不等待——事实性更新不停顿 |
| 叙事性变更未询问 | AI 替 human partner 写了描述或理由 | 回滚叙事性修改；逐条询问 human partner |
| CHANGELOG 历史被修改 | 旧条目被删除或重写 | 恢复历史条目；只允许润色最新条目 |
| 跨文档数量不一致 | README 说 5 端点，实际有 6 | 以代码为真实来源，验证实际数量后更新所有文档 |

## 好坏示例

### Good: 事实自动 + 叙事询问 + 一致性验证
```
事实性更新（自动）:
  - README.md: old/path → new/path
  - CLAUDE.md: 版本 1.2.0 → 1.3.0
叙事性更新（询问 human partner）:
  - README.md "Features": 逐条询问 → user provided / user declined
一致性验证:
  - 版本号: 一致 (1.3.0 in all docs)
  - 端点数: 验证实际 = 6 → 更新 README
```

### Bad: 不区分事实与叙事 + 跳过一致性检查
```
全部自动修改（包括叙事性内容）:
  - README.md: AI 自己写了新功能描述（未经 human partner 确认）
跳过一致性检查:
  - README 说 5 端点，代码有 6 → 未验证
  - CHANGELOG 历史条目被重写
```
