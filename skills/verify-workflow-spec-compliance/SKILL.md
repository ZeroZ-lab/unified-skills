...
## 遗漏需求（Critical）
1. [spec:LXX] <需求描述> — 影响：<...>，建议：<...>

## Scope Creep
1. <代码位置> — <说明> — 合理扩展 / 需解释

## 测试覆盖缺口
1. [spec:LXX] <验收标准> — 无对应测试
...
| 失败场景 | 处理方式 |
|---------|---------|
| 功能需求覆盖率 < 100% | 退回 build 补齐遗漏需求，提供遗漏清单和修复建议，不进入 code quality 审查 |
| 边界条件缺失 | Critical 级别 — 必须在合并前实现，否则标记不通过 |
| 验收标准缺少对应测试 | Critical 级别 — 补写测试后才可通过，无测试的功能 = 未验证的功能 |
| Scope Creep 未解释 | 要求 human partner 判断：必要则修改 spec 收入，不必要则移除代码 |
| spec 描述模糊无法验证 | 暂停审查，要求澄清 spec 意图后再继续，不做主观假设 |
| 实现方式与 spec 不一致但效果相同 | 标记为"实现方式差异"，由 human partner 判断是否通过，审查者不自行决定 |
...
```markdown
### Spec Compliance Checklist — <feature-name>

**Spec 文档**: [01-spec.md 路径]
**审查时间**: [YYYY-MM-DD]
...
**遗漏需求**（Critical）:
| # | spec 行号 | 需求描述 | 当前状态 | 影响 | 修复建议 |
|---|----------|---------|---------|------|---------|
| 1 | [L12] | [需求] | [缺失] | [影响] | [建议] |

**Scope Creep**: [列表 / 无 — 需解释或移除]
**测试覆盖缺口**: [列表 / 无]

**结论**: PASS / FAIL — [通过条件: 100% 覆盖率 / 不通过: 有 Critical 遗漏]
```
