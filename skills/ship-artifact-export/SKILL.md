...
在 `docs/features/<name>/05-ship.md` 或对应 ship 记录中写入：
...
| 说辞 | 现实 | 后果 |
|------|------|------|
| "源文件没问题，导出也没问题" | 导出是独立风险点。agent 做源文件验证，human partner 做导出验证。 | 未验证导出导致字体丢失/图片错位，接收方第一印象归零，修复 ≥ 1 轮重导出 |
| "先发这个版本，之后补 PDF" | 没有最终格式就没有完成交付。 | 格式缺失交付后无法归档，后续版本追踪断裂，返工 ≥ 重新走导出流程 |
| "文件名随便起" | 交付物需要可追踪、可归档、可复现。 | 不可追踪文件名导致版本混乱，3 个月后无法确认哪个是最终版本 |
| "截图看过了就行" | 截图不能替代打开最终 DOCX/PPTX/PDF。 | 截图无法暴露字体替换、链接失效、分页错误，上线后 ≥ N 处遗漏被发现 |
...
导出完成后应产出以下结构（写入 ship 记录）：

```markdown
## Artifact Export 记录

- **artifact_type**: [document / article / deck / visual]
- **Source**: [可编辑源文件路径]
- **Final**: [最终导出文件路径]
- **Format**: [PDF / DOCX / PPTX / PNG / SVG / HTML]
- **命名规则**: [产物-用途-版本/日期]

### 源文件验证（agent）
- 结构完整性: [通过 / 未通过 — 具体问题]
- frontmatter: [齐全 / 缺失字段]
- 链接可达: [N 个内部链接全部可达 / X 个断裂]
- 元数据: [完整 / 缺失]

### 导出验证（human partner）
- 状态: [pending / passed / failed]
- 打开验证: [页数 / 画布数 / 页面顺序 / 尺寸]
- 内容验证: [字体 / 图片 / 表格 / 链接 / speaker notes]
- 验证人: [human partner 姓名 / CI 标识]
