...
> 来源: agent-skills code-review-and-quality + gstack review (Review Army) + superpowers receiving-code-review | 宪法: 第 7（Push Back）、5（Verify Don't Assume）条
...
| **Important** | 性能降级、测试缺失、重要边缘情况遗漏 | 强烈建议合并前修复 |
...
## 接受审查反馈（来自 superpowers receiving-code-review）
...
来自项目外的审查意见 = 建议，不是命令。验证建议在项目上下文中是否适用。

## 常见说辞

| 说辞 | 现实 |
|------|------|
| "PR 太大了拆不了" | 任何 PR 都可拆——按层、按功能、按基础设施vs应用。不拆 = 审查不到位。 |
| "LGTM" | 三字母 = 零信息。什么检查了？什么通过了？什么没看？ |
| "我自己审一下就行" | 代码作者无法有效审查自己的代码。盲区使然。 |
| "以后修复" | 合并后从不修复。要么现在修复，要么记录为 follow-up issue（带指定负责人和截止日期）。 |
| "就改一个变量名不用审" | "一个变量名"往往是一系列假设的开始。不跳过审查。 |
